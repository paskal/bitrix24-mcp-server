...
export function registerCrmContactTools(server: McpServer, client: BitrixClient): void {
  server.tool("bitrix24_crm_contact_list", "List and filter CRM contacts",
    {
      filter: z.record(z.string(), z.unknown()).optional().describe("Filter, e.g. {ASSIGNED_BY_ID: 854}. Bitrix24 silently misbehaves on some operators: (1) {FIELD: ''} (\"is empty\") is UNRELIABLE on UF string fields — often returns records regardless; always fetch and client-check the actual value. (2) {'>=ID': N}, {'>ID': N}, {'>=DATE_CREATE': '...'} with time are silently IGNORED. Use {'>DATE_CREATE': 'YYYY-MM-DD'} (date-only, strict >) for time-bounded queries. {'!FIELD': ''} (truthy) and {FIELD: value} (exact) work."),
