...
export function registerCrmDealTools(server: McpServer, client: BitrixClient): void {
  server.tool("bitrix24_crm_deal_list", "List and filter CRM deals",
    {
      filter: z.record(z.string(), z.unknown()).optional().describe("Filter, e.g. {STAGE_ID: 'WON', ASSIGNED_BY_ID: 854}. Bitrix24 silently misbehaves on some operators: (1) {FIELD: ''} (\"is empty\") is UNRELIABLE on UF string fields — often returns records regardless; always fetch and client-check the actual value. (2) {'>=ID': N}, {'>ID': N}, {'>=DATE_CREATE': '...'} with time are silently IGNORED. Use {'>DATE_CREATE': 'YYYY-MM-DD'} (date-only, strict >) for time-bounded queries. {'!FIELD': ''} (truthy) and {FIELD: value} (exact) work."),
      ...
      limit: z.number().optional().describe("Max deals to return. WITHOUT this the tool returns at most 500 (10 pages x 50) and silently truncates — the response 'total' shows the true match count while 'deals' is capped at 500. For a query that matches more, pass limit >= total to fetch everything."),
    },
    async (args) => {
      try {
